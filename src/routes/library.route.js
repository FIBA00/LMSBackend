...
import { PublicLibraryListResponse, OwnerLibraryListResponse, OwnerLibraryResponse, CreateLibraryInput, UpdateLibraryInput, LibraryResponse } from "../schemas/library.schema.js";
...
libraryRoute.get("/:id", isLoggedIn, requireRole("owner"), respondWith(LibraryResponse), getUserLibraryWithId);
...
// The lesson worth keeping from this one specifically: 
// a response schema has to mirror exactly what that particular query selects — not "the model," 
// but the actual select({...}) shape, aliases included. 
// Two controllers touching the same table can legitimately need two different response schemas,
// if they select different columns or rename them. This is different from the request-side schemas 
// (CreateLibraryInput), which only need to match what you're willing to accept — 
// those can stay generic across routes more easily.
